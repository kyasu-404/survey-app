...
Ответы показываются одним списком с вертикальной прокруткой внутри таблицы. Порции по 100 записей
загружаются последовательно и автоматически до конца списка; пользовательская пагинация отсутствует.
XLSX и отчёт используют тот же полный набор ответов. Размер клиентской выборки ограничен 10 000 ответами;
ошибка загрузки любой порции не выдаётся за полный результат. При выборе всех ответов удаление также
выполняется последовательными порциями по 100 через `form-admin`.

Кнопка обновления главной запрашивает карточки и общую статистику. Статистика обновляется при наступлении
дедлайна загруженной карточки; для дедлайнов за пределами «Показать ещё» предусмотрена проверка каждые
30 секунд, пока есть формы с будущим сроком. Realtime повторно сверяет данные после подписки и
переподключения. События объединяются с максимальным ожиданием в одну секунду; уже выполняющийся запрос
завершается до следующего чтения. При уходе со страницы отложенное обновление отменяется.

Пользователи в `user-admin` и справочники организаций загружаются порциями по 1000 записей, чтобы
серверный лимит PostgREST не обрезал список. Небольшой справочник по-прежнему требует одного запроса.

Списки форм и шаблонов запрашивают на одну запись больше размера страницы. Эта lookahead-запись не показывается пользователю, а только определяет наличие следующей страницы. Поэтому пагинация этих списков не зависит от приблизительного `planned` count PostgREST.

«Показать ещё» загружает только следующую страницу. Обновление уже открытых карточек объединяет
серверные запросы: до 999 карточек плюс одна lookahead-запись за запрос, затем следующие порции при
необходимости. Все открытые карточки сохраняются; после вставок и удалений обновляется весь открытый
диапазон, чтобы не смешивать страницы из разных состояний списка. Для 200 карточек это один запрос
вместо десяти. Объём перепроверяемых данных остаётся пропорционален количеству открытых карточек.

Журнал рассылки объединяет события Realtime с задержкой 500 мс и максимальным ожиданием 1 секунду.
Изменение статуса письма обновляет только письма выбранной рассылки. Добавления обновляют список
рассылок; открытие панели и восстановление подписки выполняют сверку данных. Резервный опрос раз
в 5 секунд действует только при отсутствии Realtime и наличии незавершённых писем.

Начальная загрузка не включает SurveyJS, конструктор, ExcelJS и Sentry (Sentry подключается при наличии
DSN). `npm run build:budget` проверяет весь граф начальных JS/CSS: максимум 850 000 байт без сжатия и
230 000 байт gzip. `node scripts/measure-startup.mjs dist` из `frontend` запускает три локальных
браузерных замера при 10 Мбит/с, задержке 40 мс и замедлении CPU в 4 раза. Можно передать несколько
каталогов сборок для сравнения. Требуется установленный Chromium Playwright.

В Nginx файлы `/assets/` с хешем кешируются на год (`immutable`), HTML проходит повторную проверку
(`no-cache`), отсутствующий файл возвращает 404. Заголовки кеша заданы на уровне server, чтобы сохранить
защитные заголовки. Старые файлы с хешем следует сохранять при обновлении сборки для уже открытых вкладок.

Лимиты на один внешний IP рассчитаны на общий адрес организации: API 100 запросов/с с burst 400,
отправка ответов 20/с с burst 200, запись файлов 10/с с burst 200, соединения фронтенда — 256.
OPTIONS не расходует эти бюджеты; чтение файлов не расходует бюджет записи. Проверки доступа и квоты
хранилища продолжают действовать. Новые имена зон `_v2` позволяют изменить ключ без ошибки при reload.
Лимиты ограничивают запросы, а не гарантируют пропускную способность БД или загрузку 200 больших файлов.

`ops/prepare-magi-nginx.py --forms <текущий frontend vhost> --api <текущий API vhost> --output <каталог>`
готовит два файла для проверки и последующей установки на `magi`, сохраняя его домены, TLS и upstream.
Скрипт сам ничего не устанавливает. При внедрении оба vhost меняются вместе после резервного копирования;
перед reload проверяется полная конфигурация через `nginx -t`. Для отката возвращаются оба предыдущих файла.
`python3 ops/check-nginx-performance.py <каталог репозитория> [путь к nginx]` проверяет конфигурации в
отдельном процессе на loopback с тестовым upstream, включая reload, кеш и ограничения всплесков.

Результаты проверки от 7 сентября 2026 года: [отчёт](docs/performance-2026-09-07.md).

Списки форм и шаблонов используют cursor/keyset: курсор содержит исходные `created_at` (с микросекундами)
и `id` последней показанной карточки. RPC `list_forms_keyset` выбирает `(created_at, id) < (cursor_date, cursor_id)`;
Запрос выполняется через GET: PostgREST применяет прежние фильтры, сортировку и `LIMIT`. SQL-функция работает с правами вызывающего
пользователя и текущими RLS; ей доступны только поля карточки. Индексы `(created_at DESC, id DESC)` и
`(author_id, created_at DESC, id DESC)` уже входят в baseline. Точный общий счётчик получается отдельно через
существующий RPC статистики; пагинация не выполняет `COUNT`.

Новые записи перед курсором не сдвигают следующую страницу, а удаление строки курсора не мешает продолжению.
Обновление начинает чтение с начала списка и строит новые курсоры для всего открытого диапазона, порциями
до 999 карточек. Это последовательное чтение актуальных данных, без общей транзакционной snapshot между
HTTP-запросами: новые формы выше курсора появляются после обновления, изменение фильтруемых полей во время
чтения может изменить состав списка.

Перед установкой новой frontend-сборки примените `database/migrations/202609081200_forms_keyset_pagination.sql`.
Миграция добавляет RPC и обновляет кеш схемы PostgREST; перезапуск Nginx или Supabase не нужен.
Прежний frontend совместим с этой миграцией. Повторяемый синтетический тест и результаты:
[отчёт по keyset](docs/forms-keyset-2026-09-08.md), `ops/benchmark-forms-pagination.py`.
...
Число ответов запрашивается с `count=exact`; загрузка останавливается по точному количеству или неполной
последней порции, без запроса за пределами списка при ровно 100 или 200 ответах.
Планы поиска зависят от статистики PostgreSQL, которую обновляет `ANALYZE` и autovacuum. Обычно ручной `VACUUM ANALYZE` не нужен. Но после массового импорта, резкого роста таблиц или странных симптомов в UI/планах запросов сначала проверьте актуальность статистики:
