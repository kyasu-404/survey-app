...
Статус **«Отключён»** запрещает вход и действия сотрудника, но не закрывает созданные им формы.
Опубликованная форма продолжает принимать ответы до своего срока или лимита независимо от статуса автора.
Все активные сотрудники могут просматривать формы и ответы друг друга, включая закрытые формы, а также
скачивать вложения сохранённых ответов. Редактирование и удаление форм, удаление ответов и отправка
напоминаний по-прежнему доступны автору или администратору. Анонимные респонденты не получают доступ к
общему списку ответов и вложениям других респондентов.
...
В разделах «Все формы» и «Мои формы» кнопка рядом с поиском переключает карточки и таблицу.
Вид запоминается в браузере. Поиск, обе даты, тип, основание и обновление остаются общими;
переключение вида сохраняет фильтры, сортировку и уже загруженные страницы.

Заголовки таблицы сортируют по статусу, названию, типу/основанию, автору, дате создания и числу
ответов; повторное нажатие меняет направление. Сортировка выполняется в PostgreSQL по всему
отфильтрованному набору до ограничения страницы. Курсор содержит значение сортировки и UUID,
поэтому одинаковые значения не теряются на границах страниц. Даты сохраняют точность PostgreSQL,
число ответов сортируется как число, тип/основание — по русским подписям. Статус учитывает дедлайн
и достигнутый лимит ответов. Время проверки дедлайна фиксируется для цепочки страниц.

Обычный порядок «сначала новые» использует прежний `list_forms_keyset`; остальные варианты —
`list_forms_sorted` с правами вызывающего сотрудника и RLS. При изменениях самих ключей сортировки
(например, поступлении ответов) список согласуется существующим механизмом обновления.
Меню действий, открытие формы и переход к ответам доступны в обоих видах.
...
Удаление одной организации и «Удалить все» архивируют записи: они исчезают из действующего справочника и новых списков выбора, но сохраняют UUID и названия в старых ответах, XLSX, HTML и документах. Повторное добавление или импорт по совпадению **типа ОУ + номера + алиаса** восстанавливает запись с прежним UUID и обновляет email; для УДОД номер остаётся пустым. Изменение типа, номера или алиаса при импорте считается другой организацией.

Архивные ОУ исключены из отчёта о несдавших и новых рассылок. При редактировании сохранённого ответа можно оставить его прежнюю архивную ОУ либо выбрать действующую; использовать архивную ОУ в новом ответе или другом поле нельзя. Уже физически удалённые записи эта миграция не восстанавливает.
...
На странице результатов кнопка **«Документы»** открывает макеты XLSX и DOCX.
В ONLYOFFICE выберите ячейку или место в тексте, затем вопрос в панели
**«Данные формы»**. Вставляется поле с ID вопроса, без сводных вычислений.
**«Сформировать документы»** создаёт отдельный файл для каждого ответа
и открывает вкладку «Результат»: 20 ответов → 20 XLSX (либо 20 DOCX для текстового макета).
Результаты сохраняются; можно скачать отдельный документ или весь ZIP.
Исходный макет остаётся неизменным. Можно выбрать все ответы или отмеченные
на странице результатов; вопрос для имени файла выбирается отдельно.

Администратор настраивает Document Server и проверяет подключение в разделе
**«Настройки → ONLYOFFICE»**. Инструкция установки и ограничения первой версии:
[office-service/README.md](office-service/README.md).

Меню текущего пользователя расположено внизу боковой панели: нажмите имя, чтобы
выбрать тему или выйти. Иконки автоматически меняются для светлой и тёмной темы.
Ежедневная и ручная очистка охватывают также осиротевшие объекты ONLYOFFICE старше
7 дней; используемые макеты, результаты и файлы в периоде ожидания не удаляются.
