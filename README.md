...
- После появления первого ответа JSON-схема исходной формы становится неизменяемой на уровне БД. При открытии конструктора предлагается создать копию формы без ответов.
...
После миграции повторно разверните Edge Function `form-admin`, поскольку в ней добавлено действие `delete-responses`.
...
В конструкторе SurveyJS справочник представлен отдельным защищённым элементом **«Организация»**. Внутри он работает как выпадающий список с поиском, но источник и технические свойства не доступны автору формы. При сохранении формы можно выбрать включённые типы организаций; по умолчанию выбраны школы и детские сады. После первого ответа этот набор, как и структура формы, становится неизменяемым.
...
Удаление формы выполняется через Edge Function `form-admin`, чтобы вместе со строкой `forms` удалить связанные объекты из bucket `survey-files` и фоновые изображения из `survey-assets`. Функция принимает JWT текущего пользователя, разрешает удаление автору формы или администратору, получает пути файлов ответов из `response_file_references`, очищает оба bucket через Storage API, затем удаляет форму.
