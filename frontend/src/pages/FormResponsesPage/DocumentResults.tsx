...
import {officeRequest,downloadOfficeResult,type OfficeResult} from '../../entities/office/api';
...
export function DocumentResults({formId,focusId}:{formId:string;focusId?:string}){
 ...
 const query=useQuery({queryKey:['office-results',formId],queryFn:()=>officeRequest<{results:OfficeResult[]}>(`/forms/${formId}/results`),staleTime:0});
 ...
 return <div className="office-result-list">
  ...
  {query.data?.results.length===0 && <p className="office-empty">Результатов пока нет. Сформируйте документы из макета.</p>}
  {query.data?.results.map((result,index)=><details className="office-result-batch" key={result.id} open={focusId?result.id===focusId:index===0}>
   <summary><strong>{result.name}</strong><span>{result.files.length} файлов · {new Date(result.created_at).toLocaleString('ru-RU')} · {result.author_name || 'Сотрудник'}</span></summary>
