...
import {officeRequest,generateOfficeDocuments,type OfficeDocument,type OfficeResult} from '../../entities/office/api';
import {getErrorMessage} from '../../shared/lib/error';
export function GenerateDocuments({doc,total,selectedIds,onClose,onGenerated}:{doc:OfficeDocument;total:number;selectedIds:string[];onClose:()=>void;onGenerated:(result:OfficeResult)=>void}){
 ...
 return <section className="office-generation" aria-label="Формирование документов">
  ...
  <p>Один ответ → один {doc.file_type==='xlsx'?'XLSX':'DOCX'}. Используется последняя сохранённая версия. Готовые файлы появятся во вкладке «Результат».</p>
  ...
  <button type="button" className="responses-export-button responses-html-button" disabled={busy || count===0 || count>500} onClick={()=>{setBusy(true);setError('');void generateOfficeDocuments(doc,scope==='selected'?selectedIds:undefined,nameQuestion || undefined).then(onGenerated).catch(e=>setError(getErrorMessage(e,'Не удалось сформировать документы'))).finally(()=>setBusy(false));}}>{busy?'Формирование архива…':`Сформировать ${count} документов`}<img src={useIcon} alt="" aria-hidden="true" className="toolbar-icon"/></button>
